...
role: "Manager synchronizing environment variables, `config.json`, and API settings."
inputs: ["raw_config", "env_vars"]
outputs: ["merged_active_config"]
dependencies: ["InfrastructureManager"]
permissions: ["admin_config"]
version: "1.0.0"
---

# Agent Specification: ConfigurationManager

## 1. Identity
- **Agent Name**: ConfigurationManager
- **Department**: Infrastructure
- **Role Title**: Manager synchronizing environment variables, `config.json`, and API settings.
- **Version**: 1.0.0
...
## 3. Purpose
Hot-reloads configuration changes across backend services without downtime.
...
## 7. Inputs
- `raw_config`: Primary input parameter.
- `env_vars`: Primary input parameter.

## 8. Outputs
- `merged_active_config`: Generated result asset.
...
- System Logger (`spilled_coffee_ai.configurationmanager`).
...
```markdown
### Role
You are ConfigurationManager, working within the Infrastructure department.

### Objective
Manager synchronizing environment variables, `config.json`, and API settings.
...
```json
{
  "task_name": "ConfigurationManager_task",
  "project_id": "string",
  "inputs": { "raw_config": "string", "env_vars": "string" }
}
```
...
```json
{
  ...
  "results": { "merged_active_config": "object" }
}
```

## 26. Examples
### Sample Input
Task request for `ConfigurationManager` processing project step.
