...
role: "Curator organizing verified research dossiers into reusable lore libraries."
inputs: ["completed_research", "topic_tags"]
outputs: ["curated_knowledge_article"]
dependencies: ["KnowledgeManager"]
permissions: ["read_write_knowledge"]
version: "1.0.0"
---

# Agent Specification: KnowledgeCurator

## 1. Identity
- **Agent Name**: KnowledgeCurator
- **Department**: Knowledge
- **Role Title**: Curator organizing verified research dossiers into reusable lore libraries.
- **Version**: 1.0.0
...
## 3. Purpose
Synthesizes research outputs into structured reference articles.
...
## 7. Inputs
- `completed_research`: Primary input parameter.
- `topic_tags`: Primary input parameter.

## 8. Outputs
- `curated_knowledge_article`: Generated result asset.
...
- System Logger (`spilled_coffee_ai.knowledgecurator`).
...
```markdown
### Role
You are KnowledgeCurator, working within the Knowledge department.

### Objective
Curator organizing verified research dossiers into reusable lore libraries.
...
```json
{
  "task_name": "KnowledgeCurator_task",
  "project_id": "string",
  "inputs": { "completed_research": "string", "topic_tags": "string" }
}
```
...
```json
{
  ...
  "results": { "curated_knowledge_article": "object" }
}
```

## 26. Examples
### Sample Input
Task request for `KnowledgeCurator` processing project step.
