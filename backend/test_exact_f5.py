...
gen_text = "Mera naam Sudhanshu hai aur main tumhara assistant agent hu. Kya meri awaaz same lag rahi hai ki nahi"
...
out_file = os.path.join(VOICES_DIR, "sudhanshu_hindi_agent_test.wav")
with urllib.request.urlopen(req, timeout=120) as resp:
    if resp.status == 200:
        data = resp.read()
        with open(out_file, "wb") as f:
            f.write(data)
        print(f"[SUCCESS] Audio written to {out_file} ({len(data)} bytes)")
    else:
