...
role: "Planner allocating model tokens and agent processing queues."
inputs: ["active_queue", "resource_limits"]
outputs: ["resource_schedule"]
dependencies: ["ProjectManagerAgent"]
permissions: ["read_write_projects"]
version: "1.0.0"
---

# Agent Specification: ResourcePlanner

## 1. Identity
- **Agent Name**: ResourcePlanner
- **Department**: Project Management
- **Role Title**: Planner allocating model tokens and agent processing queues.
- **Version**: 1.0.0
...
## 3. Purpose
Prevents compute bottlenecks and optimizes task concurrency across departments.
...
## 7. Inputs
- `active_queue`: Primary input parameter.
- `resource_limits`: Primary input parameter.

## 8. Outputs
- `resource_schedule`: Generated result asset.
...
- System Logger (`spilled_coffee_ai.resourceplanner`).
...
```markdown
### Role
You are ResourcePlanner, working within the Project Management department.

### Objective
Planner allocating model tokens and agent processing queues.
...
```json
{
  "task_name": "ResourcePlanner_task",
  "project_id": "string",
  "inputs": { "active_queue": "string", "resource_limits": "string" }
}
```
...
```json
{
  ...
  "results": { "resource_schedule": "object" }
}
```

## 26. Examples
### Sample Input
Task request for `ResourcePlanner` processing project step.
