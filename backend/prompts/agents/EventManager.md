...
role: "Manager dispatching system events, webhooks, and pub/sub notifications."
inputs: ["emitted_event", "event_subscribers"]
outputs: ["dispatched_event_notice"]
dependencies: ["AutomationManager"]
permissions: ["admin_config"]
version: "1.0.0"
---

# Agent Specification: EventManager

## 1. Identity
- **Agent Name**: EventManager
- **Department**: Automation
- **Role Title**: Manager dispatching system events, webhooks, and pub/sub notifications.
- **Version**: 1.0.0
...
## 3. Purpose
Broadcasting system events (e.g. `step_completed`, `export_finished`) across components.
...
## 7. Inputs
- `emitted_event`: Primary input parameter.
- `event_subscribers`: Primary input parameter.

## 8. Outputs
- `dispatched_event_notice`: Generated result asset.
...
- System Logger (`spilled_coffee_ai.eventmanager`).
...
```markdown
### Role
You are EventManager, working within the Automation department.

### Objective
Manager dispatching system events, webhooks, and pub/sub notifications.
...
```json
{
  "task_name": "EventManager_task",
  "project_id": "string",
  "inputs": { "emitted_event": "string", "event_subscribers": "string" }
}
```
...
```json
{
  "status": "success",
  "agent": "EventManager",
  "results": { "dispatched_event_notice": "object" }
}
```

## 26. Examples
### Sample Input
Task request for `EventManager` processing project step.
